...
public class Teacher extends JFrame {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    PreparedStatement stmt = conn.prepareStatement(
                            "INSERT INTO external (ID,name, age,date,Time,Account,Charge,job) VALUES (?, ?, ?,?,?,?,?,?)");
    ...
    public void select() {
        ...
        addButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    PreparedStatement stmt = conn.prepareStatement(
                            "INSERT INTO teacherdata (Id,TeacherName,ExternalName,Subject,Date,Time,TotalStudent,Charge,Bonus,Total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public void View() {
        ...
        JTable table;
        PreparedStatement pst;
        String[] columnNames = { "ID", "TeacherName", "ExternalName", "Subject", "Date", "Time", "TotalStudent",
                "Charge", "Bonus", "Total" };
        ...
        try {
            ...
            while (rs.next()) {
                ...
                model.addRow(new Object[] { ID, TeahcherName, ExternalName, Subject, Date, Time, TotalStudent, Charge,
                        Bonus, Total });
                i++;
            }
            if (i < 1) {
